RP_test_CI_arima <- function(reg.m, reg.y, data, npermute,m,tc1,tc2,tc3,m_pred, y_pred) {
  rho.y <- reg.y$coef[["ar1"]]
  rho.m <- reg.m$coef[["ar1"]]
  
  u_y <- residuals(reg.y)
  u_m <- residuals(reg.m)
  
  #y_pred <- data$y - residuals(reg.y)
  #m_pred <- data$m - residuals(reg.m)
  MY <- t(sapply(1:npermute, function(i) {
    ...
    n_y_perm <- numeric(length(u_y_perm))
    n_m_perm <- numeric(length(u_m_perm))
    
    # initial value
    n_y_perm[1] <- u_y_perm[1]
    n_m_perm[1] <- u_m_perm[1]
    
    for (t in 2:length(u_y_perm)) {
      n_y_perm[t] <- rho.y * n_y_perm[t-1] + u_y_perm[t]
      n_m_perm[t] <- rho.m * n_m_perm[t-1] + u_m_perm[t]
    }
    
    # New data generating : ystar & mstar
    y_residuals <- n_y_perm
    m_residuals <- n_m_perm
    
    ystar <- y_pred + n_y_perm
    mstar <- m_pred + n_m_perm
    
    reg.m_perm <- try(stats::arima(mstar,order=c(1,0,0),xreg=data[,c('phase1','phase2','phase3')],method=c("ML")),silent = T)
    reg.y_perm <- try(stats::arima(ystar,order=c(1,0,0),xreg=data[,c('phase1','phase2','phase3','m','phase1m','phase2m','phase3m')],method=c("ML")),silent = T)
    #y_perm_coef <- coef(y_perm)
    if (length(reg.m_perm)>1 & length(reg.y_perm)>1) {
      mcoef=reg.m_perm$coef[-1];m_coef=c(mcoef[1],0, mcoef[2],0, mcoef[3],0, mcoef[4],0)
      ycoef=reg.y_perm$coef[-1];y_coef=c(ycoef[1],0, ycoef[2],0, ycoef[3],0, ycoef[4],0, ycoef[5:8])
      c(m_coef,y_coef)
    }else{ c(rep(NA,20)) }
