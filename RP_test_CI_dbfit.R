RP_test_CI_dbfit <- function(reg.m, reg.y, data, npermute,m,tc1,tc2,tc3) {
  ...
  MY <- t(sapply(1:npermute, function(i) {
    ...
    #data$ones=1
    reg.m_perm <- try(DBfit::dbfit(x=data[,c('ones','phase1','phase2','phase3')], y=data[,"mstar"], arp=1, method="OLS"),silent = T)
    reg.y_perm <- try(DBfit::dbfit(x=data[,c('ones','phase1','phase2','phase3','m','phase1m','phase2m','phase3m')], y=data[,"ystar"], arp=1, method="OLS"),silent = T)
    
    if (length(reg.m_perm)>1 & length(reg.y_perm)>1) {
      mcoef=reg.m_perm$coef;m_coef=c(mcoef[1],0, mcoef[2],0, mcoef[3],0, mcoef[4],0)
      ycoef=reg.y_perm$coef;y_coef=c(ycoef[1],0, ycoef[2],0, ycoef[3],0, ycoef[4],0, ycoef[5:8])
      c(m_coef,y_coef)
    }else{ c(rep(NA,20)) }
